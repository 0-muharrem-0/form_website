<?php
$page = isset($page) ? $page : 1; // Varsayılan sayfa 1 olarak ayarlandı
global $panelId; // $panelId değişkenini global yaparak dahil ediyoruz
global $totalPages5; // $totalPages5 değişkenini global yaparak dahil ediyoruz
?>
...
<div class="pagination">
    <?php if ($page > 1): ?>
        <button data-page="<?= $page - 1 ?>" data-panel="panel5"><i class="fas fa-chevron-left"></i> Önceki</button>
    <?php endif; ?>
    <?php for ($i = 1; $i <= $totalPages5; $i++): ?>
        <button data-page="<?= $i ?>" data-panel="panel5" class="<?= $i === $page ? 'active' : '' ?>"><i class="fas fa-circle"></i> <?= $i ?></button>
    <?php endfor; ?>
    <?php if ($page < $totalPages5): ?>
        <button data-page="<?= $page + 1 ?>" data-panel="panel5">Sonraki <i class="fas fa-chevron-right"></i></button>
    <?php endif; ?>
</div>
