...
<div class="pagination">
    <?php if ($page > 1): ?>
        <button data-page="<?= $page - 1 ?>" data-panel="panel2"><i class="fas fa-chevron-left"></i> Önceki</button>
    <?php endif; ?>
    <?php for ($i = 1; $i <= $totalPages1; $i++): ?>
        <button data-page="<?= $i ?>" data-panel="panel2" class="<?= $i === $page ? 'active' : '' ?>"><i class="fas fa-circle"></i> <?= $i ?></button>
    <?php endfor; ?>
    <?php if ($page < $totalPages1): ?>
        <button data-page="<?= $page + 1 ?>" data-panel="panel2">Sonraki <i class="fas fa-chevron-right"></i></button>
    <?php endif; ?>
</div>
